...
import { booksAPI, ordersAPI, apiCall } from '../services/api';
...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setStats(prev => ({
        ...prev,
        totalOrders: ordersResponse.data.data.statistics.totalOrders,
        totalRevenue: ordersResponse.data.data.statistics.totalRevenue
      }));
      ...
      try {
        const usersResponse = await apiCall.get('/admin/users');
  ...
  const handleUserStatusToggle = async (userId, currentStatus) => {
    try {
      await apiCall.put(`/admin/users/${userId}`, {
  ...
  const handleBookStatusToggle = async (bookId, currentStatus) => {
    try {
                    ...
                    <tr key={user._id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          ...
                          {user.role === 'seller' && <FaStore className="mr-1" />}
                          {user.role}
                        </span>
                      </td>
                  ...
                  {books.map((book) => (
                    <tr key={book._id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {book.seller?.sellerInfo?.businessName || 
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex space-x-2">
                          ...
                          <button
                            onClick={() => handleBookStatusToggle(book._id, book.isActive)}
                            className={`${book.isActive ? 'text-red-600 hover:text-red-900' : 'text-green-600 hover:text-green-900'}`}
                          >
                            {book.isActive ? <FaBan /> : <FaCheck />}
                          </button>
                        </div>
                      </td>
                    </tr>
